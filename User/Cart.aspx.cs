    ...
    public partial class WebForm2 : System.Web.UI.Page
    {
            ...
            if (e.CommandName == "checkout")
            {
                ...
                for (int item = 0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType == ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        int productQunatity = Convert.ToInt32(_productQuantity.Value);

                        if (productQunatity > cartQuantity && productQunatity > 2)
                        {
        ...
        protected void rCartItem_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                decimal callTotalPrice = Convert.ToDecimal(productPrice.Text) * Convert.ToDecimal(quantity.Text);
                totalPrice.Text = callTotalPrice.ToString();
                grandTotal += callTotalPrice;
            }
        ...
        private sealed class CustomTemplate : ITemplate
        {
            ...
            public void InstantiateIn(Control container)
            {
                ...
                {
                    var footer = new LiteralControl("<table class='table'><tbody><tr><td colspan='5'><b>Your cart Is Empty..</b><a href='Store.aspx' class='btn btn-dark ml-2'>Continue shopping</a></td></tr></tbody></table>");
