    ...
    public partial class WebForm4 : System.Web.UI.Page
    {
        ...
        protected void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            if (fuUserImage.HasFile)
            {
                if (Utils.isValidExtension(fuUserImage.FileName))
                {
                    ...
                    imgPath = "Images/User/" + fuUserImage.ToString() + fileExtension;
                    fuUserImage.PostedFile.SaveAs(Server.MapPath("~/Images/User/") + fuUserImage.ToString() + fileExtension);
                    cmd.Parameters.AddWithValue("@ImgUrl", imgPath);
            ...
            if (isValidToExecute)
            {
                ...
                try
                {
                    ...
                    cmd.ExecuteNonQuery();
                    actionName = userId == 0 ? 
                     "registration is successfull! <b><a href='Login.aspx'>Click Here</a></b> to do login" : "details updated successfull! <b><a href='Profile.aspx'>Can check here</a></b>";
                    lblMsg.Visible = true;
                ...
                catch (SqlException ex)
                {
                    ...
                    {
                        lblMsg.Visible = true;
                        lblMsg.Text = "<b> " + txtUsername.Text.Trim() + "</b> username already exist, try another one..!";
                        lblMsg.CssClass = "alert alert-danger";
                    }
                }
                catch (Exception ex)
        ...
        void getUserDetails()
        {
            ...
            if (dt.Rows.Count == 1)
            {
                ...
                txtMobile.Text = dt.Rows[0]["Umobile"].ToString();
                txtEmail.Text = dt.Rows[0]["Uemail"].ToString();
                txtAddress.Text = dt.Rows[0]["Address"].ToString();
                txtPostCode.Text = dt.Rows[0]["PostCode"].ToString();
                imgUser.ImageUrl = string.IsNullOrEmpty(dt.Rows[0]["ImgUrl"].ToString()) ? "../Images/No_Image.png" : "../" + dt.Rows[0]["ImgUrl"].ToString();
                imgUser.Height = 200;
                imgUser.Width = 200;
